...
Original from https://github.com/gurkirt/realtime-action-detection
Updated by Jiaojiao Zhao 
"""
...
def make_lists(rootpath, imgtype, split=1, fulltest=False):
    ...
    # each action class has its own ratios
    ...
    for vid, videoname in enumerate(sorted(database.keys())):
        ...
        actidx = database[videoname]['label']  # actidx is the class number of action
        istrain = True
        step = ratios[actidx]
        numf = database[videoname]['numf'] #number of frames
        lastf = numf-1
...
class UCF24Detection(data.Dataset):
    ...
    def __init__(self, root, image_set, transform=None, target_transform=None,
        ...
        self._imgpath1 = os.path.join(root, input_type)
        self._imgpath2 = os.path.join(root, 'fastOF-images')
        self.ids = list()
    ...
    def __getitem__(self, index):
        im,flo, gt, img_index = self.pull_item(index)
       # image = im[:,:,0:3]
       # flow = im[:,:,3:6]
        return im,flo, gt, img_index
    ...
    def pull_item(self, index):
        ...
        img_name1 = self._imgpath1 + '/{:s}/{:05d}.jpg'.format(videoname, frame_num)
        img_name2 = self._imgpath2 + '/{:s}/{:05d}.jpg'.format(videoname, frame_num)
        # print(img_name)
        ...
        return torch.from_numpy(image).permute(2, 0, 1),torch.from_numpy(flow).permute(2,0,1), target, index
...
def detection_collate(batch):
    ...
    for sample in batch:
        ...
        flos.append(sample[1])
        targets.append(torch.FloatTensor(sample[2]))
        image_ids.append(sample[3])
    return torch.stack(imgs, 0), torch.stack(flos,0), targets, image_ids
